...
def is_adoption_and_foster(row):
    if row.outcometype == "Adoption" and row.outcomesubtype == "Foster":
        return True
    elif row.outcometype == "Adoption" and row.outcomesubtype == "In Foster":
        return True
    else:
        False
...
for row in table_from:
    if not is_adoption_and_foster(row):
        new_row = OrderedDict()
...
rows.export_to_csv(table_to, "clean_data3_no_color_no_breed_no_foster_cat.csv")
...
rows.export_to_csv(table_to, "clean_data3_no_color_no_breed_no_foster_dog.csv")
